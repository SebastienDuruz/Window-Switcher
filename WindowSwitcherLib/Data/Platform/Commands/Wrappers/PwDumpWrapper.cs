...
public sealed class PwDumpWrapper() : CommandBase("pw-dump"), IPwDumpWrapper
{
    ...
    {
        if (LinuxDependencies.IsPwDumpAvailable) return ExecuteWithArguments(args, timeoutMs: 2_500);
    ...
    {
        if (LinuxDependencies.IsPwDumpAvailable) return ExecuteWithArguments(string.Empty, timeoutMs);
        LinuxDependencies.ReportMissingOnce("pw-dump");
        return string.Empty;
